        ...
        <div className='flex flex-col gap-9'>
            <div className="flex justify-between items-start">
                <div className="flex gap-5 items-start">
                    ...
                    <div className="flex flex-col gap-3">
                        <p className="text-white-1 text-heading3-bold max-sm:text-heading4-bold">
                ...
                {user.id !== userData.clerkId && (
                    isFollowing ? (
                        <PersonRemove onClick={()=>{handleFollow()}} sx={{ color: "#7857FF", cursor: "pointer", fontSize: "40px" }} />
                    ) :
                        (
                            <PersonAddAlt onClick={()=>{handleFollow()}} sx={{ color: '#7857FF', cursor: 'pointer', fontSize: "40px" }} />
                        )
                )}
            ...
            <div className="flex gap-6">
                {tabs.map((tab)=>(
                    ...
                    href={`/profile/${userData._id}/${tab.link}`}>{tab.name}</Link>
                ))}
            </div>
        </div>
    )
