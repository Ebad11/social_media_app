import { Bookmark, BookmarkBorder, BookmarkOutlined, BorderColor, Favorite, FavoriteBorder } from '@mui/icons-material'
...
const PostCard = ({post, creator, loggedInUser }) => {
    ...
    return (
        <div className='w-full max-w-xl rounded-lg flex flex-col gap-4 bg-dark-1 p-5 max-sm:gap-2'>
            <div className='flex justify-between'>
                <Link href={`/profile/${creator._id}`}>
                    <div className="flex gap-3 items-center">
                        <Image src={creator.profilePhoto} alt='profile-photo' width={50} height={50} className='rounded-full' />
                ...
                {loggedInUser.id === creator.clerkId && (
                    <Link href={`/edit-post/${post._id}`} className='flex items-center'>
            ...
            <p className='text-base-bold text-purple-1 max-sm:text-small-semibold'>{post.tag}</p>
